...
const pool = mysql.createPool({
  ...
  charset: 'utf8mb4',
  ssl: {
    rejectUnauthorized: true, // ya false hosting ke mutabiq
  }
});
// const pool = mysql.createPool({
//   host: 'localhost',
...
export async function createProduct(product) {
  await pool.execute(
    `INSERT INTO products (id, name, category_id, price, original_price, stock, image, images, description, notes, size, gender, featured, rating, reviews, variants, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
    [
...
export async function updateProduct(id, data) {
  ...
  await pool.execute(
    `UPDATE products SET name=?, category_id=?, price=?, original_price=?, stock=?, image=?, images=?, description=?, notes=?, size=?, gender=?, featured=?, rating=?, reviews=?, variants=?, updated_at=NOW() WHERE id=?`,
    [
